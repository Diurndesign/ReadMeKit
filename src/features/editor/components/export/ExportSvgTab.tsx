/**
 * Onglet "Bannière SVG" du dialog d'export.
 *
 * Contient toute la logique SVG :
 *  - ExportPreview  : aperçu clair/sombre
 *  - MarkdownSnippet : snippet Markdown copiable
 *  - ExportOptions  : cases à cocher (sanitize, fonts, rasterize, dark)
 *  - DownloadButtons: SVG / PNG / PDF / Light+Dark
 *
 * L'orchestration async (build → embed → sanitize) est déléguée à useExportProcessor.
 */
...
function ExportPreview({
  processedSvg, isProcessing, previewBg, setPreviewBg,
}: {
  processedSvg: string
...
function DownloadButtons({
  processedSvg, svgDims, isProcessing, opts, elements, canvasBg, canvasWidth, canvasHeight,
}: {
  processedSvg: string
...
/**
 * Contenu complet de l'onglet SVG — auto-contenu avec son propre état local.
 * Le parent (ExportDialog) passe `enabled` (dialog ouvert) et `onProcessedSvg`
 * pour remonter le SVG vers l'onglet README (aperçu de la bannière).
 */
export function ExportSvgTab({
  enabled,
  onProcessedSvg,
}: {
  enabled: boolean
  onProcessedSvg: (svg: string) => void
}) {
  ...
  // Remonte le SVG traité vers ExportDialog pour le partager avec ExportReadmeTab
  useEffect(() => { onProcessedSvg(processedSvg) }, [processedSvg, onProcessedSvg])

  const setOpt = <K extends keyof ExportOpts>(key: K, val: ExportOpts[K]) =>
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      ...
      <div className="w-52 shrink-0 flex flex-col gap-5 p-5 overflow-y-auto">
        <ExportOptions opts={opts} onChange={setOpt} />
