...
import { MousePointer2, Square, Type, Circle, Undo2, Redo2, Grid3X3, ZoomIn, ZoomOut, Maximize2, Download, LayoutTemplate } from 'lucide-react'
...
function useExportSVG() {
  ...
  return () => {
    ...
    const ox = minX - PAD; const oy = minY - PAD
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'readmekit-export.svg'; a.click()
...
export function EditorToolbar() {
  ...
  return (
    <div data-onboarding="toolbar" className="flex items-center gap-1 px-3 py-2 border-b border-[#2e2e33] bg-[#18181b]">
      ...
      <div className="flex items-center gap-0.5 mr-3">
        {tools.map(({ tool, icon, label, shortcut }) => (
          <div key={tool} data-onboarding={`tool-${tool}`}>
            <ToolButton icon={icon} label={label} shortcut={shortcut} isActive={activeTool === tool} onClick={() => setActiveTool(tool)} />
          </div>
        ))}
      </div>
      ...
      {/* Grid */}
      <ToolButton icon={<Grid3X3 size={18} />} label="Grille" isActive={showGrid} onClick={toggleGrid} />

      <div className="w-px h-6 bg-[#2e2e33] mx-1" />

      {/* Zoom */}
      <ToolButton icon={<ZoomOut size={18} />} label="Zoom out" onClick={() => setZoom(zoom / 1.2)} />
      <button onClick={resetView} title="Reset vue (Ctrl+0)"
      ...
      <button
        ...
        className={cn(
          ...
          elements.length > 0 ? 'bg-[#6366f1] text-white hover:bg-[#818cf8]' : 'bg-[#27272a] text-[#52525b] cursor-not-allowed'
        )}
      >
        <Download size={15} />
        Export SVG
      ...
      <span className="text-sm font-semibold text-[#a1a1aa] tracking-wide select-none">ReadMeKit</span>
    </div>
  )
}
