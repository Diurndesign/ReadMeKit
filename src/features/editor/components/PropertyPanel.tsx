...
function PropertyInput({ label, value, type = 'text', onChange }: { label: string; value: string | number; type?: string; onChange: (v: string) => void }) {
  return (
...
function SectionHeader({ children }: { children: React.ReactNode }) {
  return <h3 className="text-xs font-semibold text-[#a1a1aa] uppercase tracking-wider mt-4 mb-2 first:mt-0">{children}</h3>
}
...
function CommonProperties({ element, onUpdate }: { element: EditorElement; onUpdate: (u: Partial<EditorElement>) => void }) {
  return (
    <>
      ...
      <SectionHeader>Taille</SectionHeader>
      <div className="grid grid-cols-2 gap-2">
      ...
      <SectionHeader>Opacité</SectionHeader>
      <div className="flex items-center gap-2">
        <input type="range" min="0" max="1" step="0.05" value={element.opacity}
          onChange={(e) => onUpdate({ opacity: Number(e.target.value) })} className="flex-1 accent-[#6366f1]" />
        <span className="text-xs text-[#a1a1aa] w-10 text-right">{Math.round(element.opacity * 100)}%</span>
      </div>
    </>
  )
}
...
function RectProperties({ element, onUpdate }: { element: RectElement; onUpdate: (u: Partial<RectElement>) => void }) {
  return (
    <>
      <SectionHeader>Remplissage</SectionHeader>
      <ColorInput label="Couleur" value={element.fill} onChange={(v) => onUpdate({ fill: v })} />
      <SectionHeader>Bordure</SectionHeader>
      <ColorInput label="Couleur" value={element.stroke === 'transparent' ? '#000000' : element.stroke} onChange={(v) => onUpdate({ stroke: v })} />
      <PropertyInput label="Épaisseur" value={element.strokeWidth} type="number" onChange={(v) => onUpdate({ strokeWidth: Math.max(0, Number(v)) })} />
      <PropertyInput label="Rayon" value={element.cornerRadius} type="number" onChange={(v) => onUpdate({ cornerRadius: Math.max(0, Number(v)) })} />
    </>
  )
}
...
function TextProperties({ element, onUpdate }: { element: TextElement; onUpdate: (u: Partial<TextElement>) => void }) {
  return (
    <>
      <SectionHeader>Contenu</SectionHeader>
      <textarea value={element.content} onChange={(e) => onUpdate({ content: e.target.value })} rows={3}
      ...
      <SectionHeader>Texte</SectionHeader>
      <PropertyInput label="Taille" value={element.fontSize} type="number" onChange={(v) => onUpdate({ fontSize: Math.max(8, Number(v)) })} />
      <div className="flex items-center gap-2">
        <label className="text-xs text-[#a1a1aa] w-12 shrink-0">Graisse</label>
        <select value={element.fontWeight} onChange={(e) => onUpdate({ fontWeight: Number(e.target.value) })}
      ...
      <div className="flex items-center gap-2">
        <label className="text-xs text-[#a1a1aa] w-12 shrink-0">Aligné</label>
        <div className="flex gap-1">
          {(['left', 'center', 'right'] as const).map((a) => (
            <button key={a} onClick={() => onUpdate({ textAlign: a })}
              className={`px-3 h-8 text-xs rounded-md transition-colors ${element.textAlign === a ? 'bg-[#6366f1] text-white' : 'bg-[#0f0f11] text-[#a1a1aa] border border-[#2e2e33] hover:border-[#6366f1]'}`}>
              {a === 'left' ? 'G' : a === 'center' ? 'C' : 'D'}
            </button>
          ))}
        </div>
      </div>
      <SectionHeader>Couleur</SectionHeader>
  ...
  if (selectedIds.length === 0) {
    return (
      <div data-onboarding="panel" className="w-64 border-l border-[#2e2e33] bg-[#18181b] p-4 flex flex-col items-center justify-center text-center">
        <p className="text-sm text-[#a1a1aa]">Sélectionne un élément pour éditer ses propriétés</p>
        <p className="text-xs text-[#52525b] mt-2">Cliquer-glisser sur le canvas pour sélectionner plusieurs éléments</p>
      </div>
    )
  }
      ...
      <div data-onboarding="panel" className="w-64 border-l border-[#2e2e33] bg-[#18181b] overflow-y-auto">
        <div className="p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={clearSelection} title="Désélectionner tout" className="w-7 h-7 flex items-center justify-center rounded text-[#a1a1aa] hover:text-white hover:bg-[#27272a] text-xs">✕</button>
          </div>
          <p className="text-xs text-[#52525b]">
            Shift+clic pour ajouter/retirer. Glisser pour déplacer le groupe.
          </p>
          <div className="pt-3 border-t border-[#2e2e33] space-y-2">
    ...
    <div data-onboarding="panel" className="w-64 border-l border-[#2e2e33] bg-[#18181b] overflow-y-auto">
      <div className="p-4 space-y-2">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex gap-1">
            <button onClick={() => duplicateElement(selectedElement.id)} title="Dupliquer (Ctrl+D)"
              className="w-7 h-7 flex items-center justify-center rounded text-[#a1a1aa] hover:text-white hover:bg-[#27272a] text-xs">⧉</button>
            <button onClick={() => sendBackward(selectedElement.id)} title="Reculer"
              className="w-7 h-7 flex items-center justify-center rounded text-[#a1a1aa] hover:text-white hover:bg-[#27272a] text-xs">↓</button>
            <button onClick={() => bringForward(selectedElement.id)} title="Avancer"
              className="w-7 h-7 flex items-center justify-center rounded text-[#a1a1aa] hover:text-white hover:bg-[#27272a] text-xs">↑</button>
          </div>
        </div>

        <CommonProperties element={selectedElement} onUpdate={handleUpdate} />
        {selectedElement.type === 'rect' && <RectProperties element={selectedElement} onUpdate={handleUpdate} />}
        {selectedElement.type === 'circle' && <CircleProperties element={selectedElement} onUpdate={handleUpdate} />}
